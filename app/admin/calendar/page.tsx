...
export default function CalendarPage() {
  ...
  return <div className="p-4 md:p-6 lg:p-8 fade-up">
    ...
    <div className="bg-[#0D2744] rounded-[16px] p-4 mb-5">
      <div className="text-white text-sm font-bold mb-1">📅 חיבור ליומן Google — אישי</div>
      <div className="text-[#60A5FA] text-xs mb-3">כל האירועים והמשימות יופיעו ביומן שלך בזמן אמת</div>
      <div className="flex gap-2 mb-3">
        <a href={googleCalUrl} target="_blank"
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 bg-white rounded-[10px] text-xs font-bold text-[#0D2744] hover:bg-[#F0F7FF] transition-colors ${!googleCalUrl?"opacity-50 pointer-events-none":""}`}>
        ...
        <a href={icsUrl} download className="px-4 py-2.5 bg-white/20 text-white text-xs font-bold rounded-[10px] hover:bg-white/30 transition-colors">⬇ ICS</a>
      </div>
      <div className="bg-white/10 rounded-[10px] p-3">
        <div className="text-white/70 text-[10px] mb-1.5">הלינק האישי שלך (להעתקה לכל יומן)</div>
        <div className="flex gap-2">
          <div className="flex-1 bg-white/10 rounded-[8px] px-2.5 py-1.5 text-[10px] text-white/70 font-mono truncate">
            {icsUrl || "טוען..."}
          </div>
