...
export default function LoginPage() {
  ...
  const urlErr = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("error") : null
    ...
    <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(135deg,#0D2744,#1a5c8a)" }}>
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden w-[340px]">
        ...
        <div className="p-7 space-y-4">
          {/* Error messages */}
          {(urlErr === "not_authorized") && (
            <div className="bg-[#FEF3C7] text-[#92400E] rounded-lg px-3 py-2 text-xs text-center">
              כתובת הגוגל שלך אינה רשומה במערכת. צור קשר עם המנהל.
            </div>
          )}
          ...
          <button onClick={handleGoogle} disabled={googleLoading}
            className="w-full py-2.5 bg-white border-2 border-[#E2E8F0] rounded-[10px] text-sm font-semibold text-[#0D2744] hover:border-[#00488D] hover:bg-[#F0F7FF] disabled:opacity-50 transition-all flex items-center justify-center gap-3">
