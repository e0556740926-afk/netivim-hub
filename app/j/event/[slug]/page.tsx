...
function fd(d: string) {
  const dt = new Date(d)
  return `${dt.getDate()} ב${MONTHS[dt.getMonth()]}`
}

export default function EventPublicForm({ params }: { params: Promise<{ slug: string }> }) {
  ...
  async function submit() {
    ...
    if (!event) { setErr("קישור לא תקין — פנה ישירות"); return }
    ...
    const res = await fetch("/api/leads", {
      ...
      body: JSON.stringify({
        coordinator_id: event.coordinator_id || null,
        name: fullName,
        phone: form.phone,
        email: form.email,
    ...
    if (d.error === "כפילות") { setSent(true); return } // treat duplicate as success to avoid info leak
    setSent(true)
  }

  if (notFound) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "linear-gradient(160deg, #0D4A47 0%, #0D2744 60%, #1a1a2e 100%)" }}>
      <div className="text-white/60 text-sm">הקישור אינו תקין</div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10"
      ...
      <div className="text-center mb-8 max-w-lg" dir="rtl">
        <div className="text-white text-xl font-medium mb-1 opacity-90">נעים להכיר!</div>
        <div className="text-[#C9A84C] text-3xl md:text-4xl font-extrabold leading-tight mb-3">
          {event ? event.name : "טוען..."}
        </div>
        {event?.date && (
          <div className="text-white/50 text-sm">{fd(event.date)}{event.location ? ` · ${event.location}` : ""}</div>
        )}
      </div>
      ...
      <div className="w-full max-w-md" dir="rtl">
        <div className="space-y-3">
          ...
          <button onClick={submit} disabled={loading || !event}
