...
const T = { navy: "#14213D", blue: "#2E5C8A", blueBg: "#EDF2F8", slate: "#5A6472", border: "#CBD3DD", bg: "#F7F9FC", ok: "#2E6B4F", warn: "#B7791F" }
...
export default function AdviseeAccountPortal() {
  ...
  async function load() {
    ...
    try {
      const r = await fetch("/api/portal/advisee/me")
      if (r.status === 401) { router.push("/portal/advisee/login"); return }
  ...
  async function logout() {
    await fetch("/api/portal/advisee/verify", { method: "DELETE" })
    router.push("/portal/advisee/login")
  }
  async function sendMessage() {
    if (!message.trim()) return
    await fetch("/api/portal/advisee/me", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message }) })
    setMessage(""); setSent(true); setTimeout(() => setSent(false), 3000)
  }

  if (loading) return <div style={{ padding: 32, textAlign: "center" }}>טוען...</div>
  if (error || !data) return <div style={{ padding: 32, textAlign: "center", color: "#C0392B" }}>שגיאה. נסה להתחבר שוב.</div>
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "Assistant, Heebo, sans-serif", background: "#EEF1F6", minHeight: "100vh", padding: 24, display: "flex", justifyContent: "center" }}>
      <div style={{ width: 390, maxWidth: "100%", background: T.bg, minHeight: 780 }}>
        <div style={{ background: T.navy, color: "#fff", padding: "20px 20px 16px", display: "flex", justifyContent: "space-between" }}>
          <div>
          ...
          <span onClick={logout} style={{ fontSize: 12, color: "#9AA6BE", cursor: "pointer", textDecoration: "underline", alignSelf: "flex-start" }}>יציאה</span>
        </div>
