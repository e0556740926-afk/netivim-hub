...
export default function InstitutionAccountPortal() {
  ...
  async function load() {
    ...
    try {
      const [r, ret] = await Promise.all([fetch("/api/portal/institution/me"), fetch("/api/portal/institution/me/retention")])
      if (r.status === 401) { router.push("/portal/institution/login"); return }
  ...
  async function logout() {
    await fetch("/api/portal/institution/login", { method: "DELETE" })
    router.push("/portal/institution/login")
  }

  async function submitReferralStatus(refId: number) {
    await fetch("/api/portal/institution/me/referrals", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ referral_id: refId, status: statusForm.status, reason: statusForm.reason, date: statusForm.date }) })
    setOpenReferral(null); setStatusForm({ status: "הוזמן לראיון" })
    await load()
  }
  async function confirmAllHere() {
    await fetch("/api/portal/institution/me/retention", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ confirmation_id: retention.pending.id, all_still_here: true }) })
    await load()
  }
  async function saveWithChanges() {
    await fetch("/api/portal/institution/me/retention", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ confirmation_id: retention.pending.id, all_still_here: false, left_case_ids: Array.from(leftIds) }) })
    setShowChange(false); setLeftIds(new Set())
    await load()
  }

  if (loading) return <div style={{ padding: 32, textAlign: "center" }}>טוען...</div>
  if (error || !data) return <div style={{ padding: 32, textAlign: "center", color: T.breach }}>שגיאה בטעינה. נסה להתחבר שוב.</div>
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "Assistant, Heebo, sans-serif", background: "#EEF1F6", minHeight: "100vh", padding: "24px 0", display: "flex", justifyContent: "center" }}>
      <div style={{ width: 400, maxWidth: "100%", background: T.bg, minHeight: 800 }}>
        <div style={{ background: T.navy, color: "#fff", padding: "20px 20px 24px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
