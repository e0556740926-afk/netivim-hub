...
export default function PublicSourceForm({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/lead-sources/slug?slug=${encodeURIComponent(slug)}`)
      .then(r => r.json())
      .then(d => {
        if (d.source) { setSourceLabel(d.source.label); setSourceId(d.source.id); setCoordinatorId(d.source.coordinator_id) }
      })
  }, [slug])
  ...
  async function submit() {
    ...
    if (!sourceId) { setErr("קישור לא תקין — פנה ישירות"); return }
    ...
    if (d.error === "כפילות") { setSent(true); return }
    setSent(true)
  }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10"
      ...
      <div className="text-center mb-8 max-w-lg" dir="rtl">
        ...
        <div className="text-[#C9A84C] text-3xl md:text-4xl font-extrabold leading-tight mb-4">
          את המסלול המדויק{sourceLabel ? ` דרך ${sourceLabel}` : ""}?
        </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="relative">
              <input value={form.firstName} onChange={e => setForm(f => ({ ...f, firstName: e.target.value }))} placeholder="שם פרטי"
                className="w-full px-4 py-3.5 bg-white/10 border border-white/30 rounded-[12px] text-white placeholder-white/50 text-sm focus:outline-none focus:border-[#C9A84C] focus:bg-white/15 transition-all backdrop-blur-sm" style={{ fontSize: "16px" }} />
              {!form.firstName && err && <div className="absolute -bottom-5 right-0 text-[#f87171] text-[10px]">שדה חובה</div>}
            </div>
            <input value={form.lastName} onChange={e => setForm(f => ({ ...f, lastName: e.target.value }))} placeholder="שם משפחה"
              className="w-full px-4 py-3.5 bg-white/10 border border-white/30 rounded-[12px] text-white placeholder-white/50 text-sm focus:outline-none focus:border-[#C9A84C] focus:bg-white/15 transition-all backdrop-blur-sm" style={{ fontSize: "16px" }} />
          </div>

          <input value={form.age} onChange={e => setForm(f => ({ ...f, age: e.target.value }))} type="number" placeholder="גיל הבחור"
            className="w-full px-4 py-3.5 bg-white/10 border border-white/30 rounded-[12px] text-white placeholder-white/50 text-sm focus:outline-none focus:border-[#C9A84C] focus:bg-white/15 transition-all backdrop-blur-sm" style={{ fontSize: "16px" }} />
          ...
          <input value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} type="email" placeholder="אימייל"
            className="w-full px-4 py-3.5 bg-white/10 border border-white/30 rounded-[12px] text-white placeholder-white/50 text-sm focus:outline-none focus:border-[#C9A84C] focus:bg-white/15 transition-all backdrop-blur-sm" style={{ fontSize: "16px" }} />

          <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="פרטים נוספים שתרצו להוסיף" rows={3}
            className="w-full px-4 py-3.5 bg-white/10 border border-white/30 rounded-[12px] text-white placeholder-white/50 text-sm focus:outline-none focus:border-[#C9A84C] focus:bg-white/15 transition-all resize-none backdrop-blur-sm" style={{ fontSize: "16px" }} />

          <label className="flex items-center gap-3 cursor-pointer group py-1">
            <div onClick={() => setForm(f => ({ ...f, isParent: !f.isParent }))}
          ...
          <div className="text-center text-white/30 text-xs pt-2">הפרטים שלך מאובטחים ומשמשים לצורך יצירת קשר בלבד</div>
        </div>
      </div>
    </div>
  )
