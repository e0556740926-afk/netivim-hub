...
export default function CalendarPopup({ token, label }: { token: string; label?: string }) {
  ...
  useEffect(() => {
    if (token) setUrl(`${window.location.origin}/api/calendar.ics/${token}`)
  }, [token])
      ...
      {open && (
        <div className="absolute top-full mt-2 left-0 z-50 w-80 bg-white border border-[#E2E8F0] rounded-[14px] shadow-xl p-4" dir="rtl">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setOpen(false)} className="text-[#94A3B8] hover:text-[#374151] text-sm leading-none">✕</button>
          </div>
          <div className="text-xs text-[#64748B] mb-3">
            העתק את הלינק והדבק אותו ביומן גוגל ← הגדרות ← יומנים אחרים ← הוסף לפי URL
          </div>
          ...
          <button onClick={copy} className={`w-full py-2.5 rounded-[9px] text-sm font-bold transition-colors ${copied ? "bg-[#DCFCE7] text-[#166534]" : "bg-[#0D2744] text-white hover:bg-[#00488D]"}`}>
