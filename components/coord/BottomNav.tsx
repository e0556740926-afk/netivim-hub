...
export default function BottomNav() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/tasks?name=${encodeURIComponent(user.name)}`)
      .then(r=>r.json())
      .then(d => setLate((d.tasks||[]).filter((t:any)=>t.due_date&&t.due_date.slice(0,10)<today).length))
  }, [user])

  return (
    <div className="fixed bottom-0 right-0 left-0 h-[62px] bg-white border-t border-[#E2E8F0] flex items-center justify-around px-1 z-50 shadow-[0_-4px_16px_rgba(0,0,0,0.07)]">
      {tabs.map(t => {
        const active = path.startsWith(t.path)
        return (
          <div key={t.path} onClick={() => router.push(t.path)}
            className="flex flex-col items-center gap-0.5 cursor-pointer px-2 py-1.5 relative min-w-[54px]">
            {t.badge && late > 0 && (
            ...
            {active && <div className="absolute bottom-0.5 w-5 h-0.5 rounded-full bg-[#00488D]"/>}
          </div>
        )
      })}
    </div>
